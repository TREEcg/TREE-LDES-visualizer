            ...
            if (collectionObj.view){
              for (var viewNode of collectionObj.view){
                ...
                if (!double){
                  ...
                  this.jsondata.relations_holder.push({"id":viewNode['@id']+"_relation_holder", "node_id":viewNode['@id']+"_node", "name":viewNode['@id'], "relation_count":metadata.nodes.get(viewNode['@id']).relation.length, "offsetX": this.jsondata.relations_holder.length, "expanded": "false"})
      ...
      function expandRelationHolderTrue(currentg, d){
        ...
        for (let relX of this.jsondata[d.node_id]){
          ...
          .raise();
            ...
            .text(textX)
            .attr("dy", 20)
            .attr("x", 35+svgEG.attr("items")*500)
            .attr("sortIndex", sortIndex);
            //.attr("info", JSON.stringify(relX));
          ...
          .attr("height", 25)
          .attr("x", 30+svgEG.attr("items")*500)
